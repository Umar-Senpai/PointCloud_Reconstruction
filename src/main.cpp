...
Eigen::MatrixXf ICP(Eigen::MatrixXf source_pc, Eigen::MatrixXf transformed_pc, int MAX_I, Eigen::Matrix4f transform, std::ofstream& out) {
  ...
  out << "iter: " << counter << std::endl;
  out << "time: " << duration.count() << std::endl;
  out << "MSE: " << mean_error << std::endl;
  out << "r_error: " << std::acos(((R_total.transpose() * R_gt).trace() - 1) / 2) * 180.0 / M_PI << std::endl;
  out << "t_error: " << std::sqrt((t_total - t_gt).squaredNorm()) << std::endl;

  return source_pc;
}

Eigen::MatrixXf TrICP(Eigen::MatrixXf source_pc, Eigen::MatrixXf transformed_pc, int MAX_I, Eigen::Matrix4f transform, std::ofstream& out) {
  auto time_start = std::chrono::high_resolution_clock::now();
  using my_kd_tree_t = nanoflann::KDTreeEigenMatrixAdaptor<
    Eigen::MatrixXf, 3, nanoflann::metric_L2>;
...
int main(int argc, char **argv) {
  ...
  // define output file
  std::stringstream outfile_icp;
	outfile_icp << output_file << "_icp.txt";
	std::ofstream outfileICP(outfile_icp.str());

  std::stringstream outfile_tricp;
	outfile_tricp << output_file << "_tricp.txt";
	std::ofstream outfileTRICP(outfile_tricp.str());
  ...
  Eigen::MatrixXf new_source_pc_tricp = TrICP(source_pc, transformed_pc, MAX_I, transform, outfileTRICP);

  pcl::PointCloud<pcl::PointXYZ>::Ptr new_source_cloud_icp (new pcl::PointCloud<pcl::PointXYZ> ());
  ...
  pcl::visualization::PCLVisualizer viewer_icp ("ICP");
  pcl::visualization::PCLVisualizer viewer_tricp ("Tr-ICP");

  pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> source_cloud_color_handler_icp (new_source_cloud_icp, 255, 255, 255); // White
  viewer_icp.addPointCloud (new_source_cloud_icp, source_cloud_color_handler_icp, "new_source_cloud_icp");

  pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZ> transformed_cloud_handler_icp (transformed_cloud, 255, 255, 255); // Red
  viewer_icp.addPointCloud (transformed_cloud, transformed_cloud_handler_icp, "transformed_cloud");
  ...
  viewer_icp.setPointCloudRenderingProperties (pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 2, "new_source_cloud_icp");
