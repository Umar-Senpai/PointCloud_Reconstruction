...
## Algorithms
The main C++ file in `/src` named `main.cpp` implements the different functions and algorithms required for point cloud reconstruction. The implemented algorithms are:
...
```
Here `PC1` and `PC2` are two point clouds which can be partially overlapped. It can also be same cloud as we apply transformations to the `PC2`. The `rot_angle_deg` applies a rotation along `z-axis` and `trans_dist` applies a translation in `x-axis` to `PC2`. The `noise_std` parameter refers to the standard deviation of the added gaussian noise to the points of `PC1`. We can also define the `MAX_I` i.e. maximum iterations. The output string passed will generate two output files.

### Generating results for evaluation
An example format of these files is:
```
output_icp.txt
output_tricp.txt
```
These files will contain the evaluation metrics such as:
* Processing Time
...
A sample command is given to run the code
```
./Main ../data/horse0.pcd ../data/horse1.pcd 500 1.0 20 10.0 ../results/fountain_1_20_10
```

After executing this command, our code will take the partially overlapped horse point clouds and perform both ICP and TrICP. It will then display the results in visualization windows. 

## Evaluation
To evaluate, we have taken three pairs of point clouds which represents a wolf, a fountain and a horse. 
### Wolf
The wolf point clouds have almost ~90% overlap and on one wolf, a part of neck is missing. While on the other wolf, some part of its leg is missing. We will apply the two algorithms with different parameters and observe if they are able to reconstruct the wolf.
...
The following table represents the evaluation matrices generated by the above parameters.
...
From the table, we can see that the algorithms are pretty fast for small point cloud data. Each point cloud only has ~2000 points and it takes around half a second to converge the algorithms. In case of ICP, the MSE is high but the reconstructed point cloud is good enough. But for TrICP, with 60% match factor, the MSE is almost 0. 
### Fountain
The fountain point clouds have very small overlap. We will apply the two algorithms with different parameters and observe if they are able to reconstruct the fountain.
Parameters | Ground Truth | ICP | TrICP
:-------------------------:|:-------------------------:|:-------------------------:|:-------------------------:
...
From the above images, we can observe that ICP fails to reconstruct the fountain but TrICP, given that the initial translation is small, can very well reconstruct the fountain. The following table represents the evaluation matrices generated by the above parameters.

Algorithm | Parameters | # Iterations | Time Taken (s) | Rotational Error (Deg) | Translational Error | MSE
:-------------------------:|:-------------------------:|:-------------------------:|:-------------------------:|:-------------------------:|:-------------------------:|:-------------------------:
ICP|noise: 0<br>rot: 10&deg;<br>x-trans: 10m|121|38.2054|24.5673|7.24542|0.0831441
TrICP|noise: 0<br>rot: 10&deg;<br>x-trans: 10m|123|38.7051|41.4124|9.44512|0.5135671
ICP|noise: 0<br>rot: 10&deg;<br>x-trans: 2m|109|36.7069|23.4955|7.88392|0.0820526
TrICP|noise: 0<br>rot: 10&deg;<br>x-trans: 2m|103|33.4133|6.78443|1.10636|0.00354314
ICP|noise: 0.1<br>rot: 20&deg;<br>x-trans: 2m|132|46.2394|21.7831|7.82403|0.0876612
TrICP|noise: 0.1<br>rot: 20&deg;<br>x-trans: 2m|139|47.1741|17.6116|6.89916|0.0102556

The MSE for all cases is small. As the point clouds are bigger, more iterations and time is taken by both algorithms. The ICP thinks that it has reconstructed the fountain very well given the low MSE values but the generated cloud is not a good representation of the fountain. The TrICP in this case, is more robust. 
### Horse
The horse point clouds have very small overlap, and both represent almost half of the horse, vertically. We will apply the two algorithms with different parameters and observe if they are able to reconstruct the fountain.
...
From the table above, we have increased the noise and distance, but TrICP is performing well, whereas the ICP is merging both point clouds too deep and the horse looks very thin. Let's look at the table to see the evaluation metrics.
Algorithm | Parameters | # Iterations | Time Taken (s) | Rotational Error (Deg) | Translational Error | MSE
:-------------------------:|:-------------------------:|:-------------------------:|:-------------------------:|:-------------------------:|:-------------------------:|:-------------------------:
ICP|noise: 0<br>rot: 10&deg;<br>x-trans: 10m|47|0.46|8.45712|36.6667|115.779
TrICP|noise: 0<br>rot: 10&deg;<br>x-trans: 10m|33|0.333895|9.50523|29.8562|14.4462
ICP|noise: 0<br>rot: 20&deg;<br>x-trans: 50m|41|0.427337|7.89342|37|115.777
TrICP|noise: 0<br>rot: 20&deg;<br>x-trans: 50m|28|0.308708|9.30544|30.0871|14.4468
ICP|noise: 1.0<br>rot: 20&deg;<br>x-trans: 10m|38|0.386907|8.17039|36.5703|116.785
TrICP|noise: 1.0<br>rot: 20&deg;<br>x-trans: 10m|34|0.378374|9.30037|29.9236|15.1594

We can see that the MSE in case of ICP is very high but TrICP is performing well.

## Extra: 3D Reconstruction from a series of point clouds
We have taken a series of point clouds from the warehouse objects given here: http://rgbd-dataset.cs.washington.edu/dataset/rgbd-scenes-v2/

The `main_extra.cpp` file inside `src` implements reconstruction of a series of clouds. The general command to run the code is
```
./Reconstruction PC1 PC2 MAX_I noise_std rot_angle_deg trans_dist
```
We have currently performed 3D reconstruction on office chair 1 inside the warehouse objects dataset. We are doing reconstruction using the following command
```
./Reconstruction ../office_chair/office_chair_1_1.ply ../office_chair/office_chair_1_2.ply 500 0 0 0
```
We can observe the difference by running the previous code and reconstructing only two point clouds.

### Reconstruction with two point clouds
...
And now combining all of the eight point clouds.

### Reconstruction with eight point clouds
...
This reconstruction is done with ICP. The dataset is uploaded with the repository and some results are also uploaded for reference. 
